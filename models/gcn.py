...
class GCNAbsaModel(nn.Module):
    def __init__(self, args, emb_matrix=None):
        ...
        # gcn layer
        self.gcn = GCN(args, embeddings, args.gcn_hidden_dim, args.num_layers)
    ...
    def forward(self, inputs):
        ...
        mask = mask.unsqueeze(-1).repeat(1,1,self.args.gcn_hidden_dim)    # mask for h
...
def rnn_zero_state(batch_size, gcn_hidden_dim, num_layers, bidirectional=True):
    total_layers = num_layers * 2 if bidirectional else num_layers
    state_shape = (total_layers, batch_size, gcn_hidden_dim)
