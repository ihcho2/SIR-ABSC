...
class BiGraphConvolution(nn.Module):
    ...
    def forward(self, text, adj1, adj2):
        ...
        output = F.relu(torch.matmul(adj1, hidden) / denom1 + self.bias) + hidden
        output= self.dropout(output)
        output2 = F.relu(torch.matmul(adj2, hidden2) /denom2 + self.bias2) + hidden2
...
class TD_BERT_with_GCN(nn.Module):
    def __init__(self, config, opt):
        ...
        self.dropout = nn.Dropout(opt.keep_dropout)
        self.bert_fc = nn.Linear(embedding_dim+opt.gcn_hidden_dim, output_dim)
...
class BERT_FC_GCN(nn.Module):
    def __init__(self, config, opt):
        super(BERT_FC_GCN, self).__init__()
        self.opt = opt
        n_filters = opt.n_filters  # The number of convolution kernels
        filter_sizes = opt.filter_sizes  # Convolution kernel size, multiple sizes are passed as a list
        embedding_dim = opt.embed_dim  
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, labels=None, input_t_ids=None, input_t_mask=None,
        ...
        # Concatenate the output of BERT and GCN.
        concatenated_embed = torch.cat((pooled_output, gcn_cat), dim=1)
...
class BERT_FC_TD_GCN(nn.Module):
    def __init__(self, config, opt):
        super(BERT_FC_TD_GCN, self).__init__()
        self.opt = opt
        n_filters = opt.n_filters  # The number of convolution kernels
        filter_sizes = opt.filter_sizes  # Convolution kernel size, multiple sizes are passed as a list
        embedding_dim = opt.embed_dim  
        ...
        self.dropout = nn.Dropout(opt.keep_dropout)
        self.bert_fc = nn.Linear(2*embedding_dim+opt.gcn_hidden_dim, output_dim)
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, labels=None, input_t_ids=None, input_t_mask=None,
        ...
        # Concatenate the output of BERT and GCN.
        concatenated_embed = torch.cat((pooled_output, cat, gcn_cat), dim=1)
