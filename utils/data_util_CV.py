...
class ReadData:
    def __init__(self, opt):
        ...
        if opt.model_name in ['gcls', 'gcls_er', 'gcls_moe']:
        ...
        self.train_data, self.train_eval_data, self.train_dataloader, self.train_eval_dataloader, self.train_tran_indices, self.train_span_indices = self.get_data_loader(examples=self.train_examples, type='train_data')
    ...
    def get_data_loader(self, examples, type='train_data'):
        ...
        if type == 'train_data':
            all_input_ids_train = all_input_ids[:portion]
            all_input_ids_train_eval = all_input_ids[portion:]
        ...
        if type == 'train_data':
            all_input_mask_train = all_input_mask[:portion]
            all_input_mask_train_eval = all_input_mask[portion:]
        ...
        if type == 'train_data':
            all_segment_ids_train = all_segment_ids[:portion]
            all_segment_ids_train_eval = all_segment_ids[portion:]
        ##########
...
#             all_segment_ids[i][aspect_start_idx:aspect_end_idx] = 1
        ...
        ##############################
        if type == 'train_data':
            all_label_ids_train = all_label_ids[:portion]
            all_label_ids_train_eval = all_label_ids[portion:]
        ...
        if type == 'train_data':
            all_tran_indices_train = all_tran_indices[:portion]
            all_tran_indices_train_eval = all_tran_indices[portion:]
            
            all_span_indices_train = all_span_indices[:portion]
            all_span_indices_train_eval = all_span_indices[portion:]
            
            all_input_guids_train = all_input_guids[:portion]
            all_input_guids_train_eval = all_input_guids[portion:]
            
            all_input_dg_train = all_input_dg[:portion]
            all_input_dg_train_eval = all_input_dg[portion:]
            
            all_input_dg1_train = all_input_dg1[:portion]
            all_input_dg1_train_eval = all_input_dg1[portion:]
            
            all_input_dg2_train = all_input_dg2[:portion]
            all_input_dg2_train_eval = all_input_dg2[portion:]
            
            all_input_dg3_train = all_input_dg3[:portion]
            all_input_dg3_train_eval = all_input_dg3[portion:]
            
        if type == 'train_data':
            data_train = TensorDataset(all_input_ids_train, all_input_mask_train, all_segment_ids_train, all_label_ids_train, all_input_dg_train, all_input_dg1_train, all_input_dg2_train, all_input_dg3_train, all_input_guids_train)
            
            data_train_eval = TensorDataset(all_input_ids_train_eval, all_input_mask_train_eval, all_segment_ids_train_eval, all_label_ids_train_eval, all_input_dg_train_eval, all_input_dg1_train_eval, all_input_dg2_train_eval, all_input_dg3_train_eval, all_input_guids_train_eval)
            
        else:
            data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_input_dg, all_input_dg1, 
                                 all_input_dg2, all_input_dg3, all_input_guids)
        
        if type == 'train_data':
            train_data = data_train
            train_eval_data = data_train_eval
            train_sampler = RandomSampler(train_data)
            train_eval_sampler = RandomSampler(train_eval_data)
            return train_data, train_eval_data, DataLoader(train_data, sampler=train_sampler, batch_size=self.opt.train_batch_size), DataLoader(train_eval_data, sampler=train_eval_sampler, batch_size=self.opt.train_batch_size), all_tran_indices, all_span_indices
        else:
            eval_data = data
