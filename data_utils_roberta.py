...
#    else:
    if True:
        ...
        fname = './datasets/glove/glove.840B.300d.txt'
...
#        unknownidx = 1_convert_token_to_id
        #print(realwords)
        sequence = [self.tokenizer._convert_token_to_id('<s>')]+[self.tokenizer._convert_token_to_id(w) for w in realwords]+[self.tokenizer._convert_token_to_id('</s>')]
        if len(sequence) == 0:
            sequence = [0]
...
#                 pickle.dump(tokenizer.word2idx, f)
        ...
        for item in data:
            vocab[item] = index
            index_vocab[index] = item
            index +=1
    
        self.embedding_matrix = build_embedding_matrix(vocab, embed_dim, dataset)
        if change:
