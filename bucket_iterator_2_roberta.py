...
class Instructor:
    def __init__(self, args, init_model_state_dict = None):
        ...
        if args.model_name in ['roberta_td', 'roberta_td_t_star', 'roberta_gcls', 'roberta_gcls_vdc_auto', 'roberta_gcls_step']:
...
#             self.model = RobertaForSequenceClassification(roberta_config)
        ...
        elif args.model_class == RobertaForSequenceClassification_gcls_step:
            print('args.automation_type: ', args.automation_type)
            self.model = RobertaForSequenceClassification_gcls_step.from_pretrained('roberta-base',g_pooler = args.g_pooler,
                                                                                        automation_type = args.automation_type,
                                                                                        auto_k = args.auto_k, 
...
#         torch.save(self.model.state_dict(), './roberta_rpt/init_ckpt.pkl')
        ...
        elif args.model_name in ['roberta', 'roberta_td', 'roberta_td_t_star', 'roberta_gcls', 'roberta_gcls_vdc_auto',
                                 'roberta_gcls_step']:
...
#                 self.scheduler.step()
            ...
            for step, batch in enumerate(tqdm(self.dataset.train_dataloader, desc="Training")):
                ...
                if self.opt.model_name in ['roberta_lcf', 'roberta_lcf_td']:
                    ...
                    input_ids_lcf_global = input_ids_lcf_global.to(self.opt.device)
                    input_ids_lcf_local = input_ids_lcf_local.to(self.opt.device)
                elif self.opt.model_name in ['roberta_td', 'roberta_td_t_star', 'roberta_gcls', 'roberta_gcls_vdc_auto']:
                ...
                elif self.opt.model_name in ['roberta_gcls_step']:
                    input_ids = input_ids.to(self.opt.device)
                ...
                if self.global_step % 100 == 0:
                    if self.opt.model_name in ['roberta_td', 'roberta_td_t_star', 'roberta_gcls', 'roberta_gcls_vdc_auto',
                                               'roberta_gcls_step']:
                        print('-'*77)
...
#                     loss = loss + max(0, 0.2 - sum(sg_loss)/len(sg_loss))
                ...
                elif self.opt.model_class in [RobertaForSequenceClassification_gcls_step]:
                    sg_loss, output_, _ = self.model(input_ids, labels = label_ids,
                                              extended_attention_mask = train_extended_attention_mask, 
        ...
        for batch in tqdm(d_loader, desc="Evaluating"):
            ...
            with torch.no_grad():
                if self.opt.model_class in [BertForSequenceClassification, CNN]:
                    ...
                    sg_loss, output_, eval_vdc_list = self.model(input_ids, labels = label_ids,
                                              extended_attention_mask = extended_att_mask,
                                              VDC_info = eval_VDC_info)[:3]
                    
                    loss, logits = output_[:2]
                    if eval_vdc == None:
                        eval_vdc = eval_vdc_list
                    else:
                        eval_vdc = torch.cat((eval_vdc, eval_vdc_list), dim =0)
...
if __name__ == "__main__":
    ...
    model_classes = {
        ...
        'roberta_gcls_step': RobertaForSequenceClassification_gcls_step,
